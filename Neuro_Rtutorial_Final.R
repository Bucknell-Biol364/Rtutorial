...
#R is a program for data science. It is an extremely useful tool for manipulating,
#visualizing, and investigating data. It will allow you to explore data to formulate 
#hypotheses and then employ statistical tests to assess them.
...
#To create a working directory you should first create a folder in a location on 
#this computer that you can readily access. We recommend creating the folder in 
#Documents. Next select the "Session" tab located above this window near where it
#says "File" and "Edit". Next click "Set Working Directory" and select 
#"Choose Directory..." from the drop down menu. Within the file explorer that 
#will appear, navigate to where you stored the new folder you just created. 
#Select the folder and click "Ok" to set it as your working directory. In this 
#case you will want to move the folder you cloned from github to your newly 
#established working directory so that you can readily access the files you will 
#need.
...
#will be using a data set from a lab similar to those you will be performing this
#semester. The data set "Neuro_253_Cleaned_Data" is a series of extracellular 
#neuronal recordings from crayfish. It includes recordings of "spikes" which 
#refers to the firing of the neurons in the units of action potentials per second
#measured both before and after treatment with drugs. 

#This file is in the .csv format, an abbreviation for "comma separated values". 
#You can create such a file from an excel document. After entering and organizing
#data in an excel spreadsheet you can "save as" with the file extension .csv. You
#will need to save your data sets within the folder you have set as your working
#directory so that it will be accessible within R. After you have moved all of 
#the cloned files to your working directory you should see the 
#"Neuro_253_Cleaned_Data" dataset within the directory.

#Once the Neuro_253_Cleaned_Data.csv file is in your working directory you will
#want to read it into the R session as a data frame. A data frame is a table of
#information that can be of various types which is convenient when data are not
#of uniform classes. The read.csv function automatically takes the file name you
#provide in quotes and creates a data frame named whatever you provide to the
#left of the command containing the information from your data file. Below 
...
#You now have a data set in R that is ready to be manipulated using the commands
#within this document. 


#General Syntax:
  
#  The '<-' symbol incdicates assignment and allows you to create a variable 
#(for instance in the previous code you can see that we used this to create the
#data frame for our data set)

#To access a specific variable you can use the dollar sign. For instance to 
#select a specific drug within this data set you would want to use
#Neuron_Recording_Data$drug name. 

#R has many built in functions available that can be used to perform some sort of
#action on existing variables. To perform a function on a variable you use the
...
#and tail (last five rows) of your dataset. You can also view the full dataset by
#typing the name of the file and running that line of code.

#Another issue you should determine from the start is if there are missing values
#in your dataset. If it is a dataset you have collected yourself this may be easy
...
#let's consider you were determining if there is a difference test scores in 
#two separate classes. The data could be set up such that Class A is the title
#of the first column and below it are the test scores of 10 different students,
#and Class B is the title of the second column with it's students test scores 
#below it. R would treat the first row as one observation when it is actually 
#two, one test score from class A and one score from test B. Stacked data will
#have the title of the first column be Grades or Values, and the title of the
#second column Class. So the rows would then be set up as a single observation 
...
#range. When trying to find the mean, median, etc. of a specific variable 
#you must first specify the dataset you are looking at and then the variable 
#you want to focus on. This is done in the format DataFile$Variable. To start 
#off lets look at the variable Control_Spikes
...
#Try these commands with your dataset and pay close attention to what it tells
#you about this dataset in particular. What does it mean if the data has a 
#large range? a small interquartile distance? Are the mean and median similar?
...
#You can check to see if the data is normal using the simple.eda function in the
#UsingR package. The simple.eda function will show you a histogram, boxplot,
#and normal Q-Q plot of the data. 
...
#The last two graphs show the distributions for control and experimental spikes
#respectively. Let's look a little closer at the control spikes data. 
...
# numeric or categorical (factor). You should also make decisions based on 
# if your numeric variable is discrete or continuous.

#The zip folder downloaded for this tutorial includes a helpful guide 
# (KylaJacksonPlotTypes.png) for determining which visualization is best 
# given your variables of interest.
...
# cex.lab used to magnify text of x and y axis labels
...
# In the space below, try to practice with other visualizations and variables 
# within the Iris data set. Then, move on to the next section!
...
#Now that we are familiar with the tools to explore our data let's put them into
#practice to formulate and test a hypothesis. 

#For practice let's test the hypothesis that treating with the drug pilocarpine
#increases spike rate.

# To start it would be helpful to group the data based on the variables you are
# most interested in. This is a paired data set meaning we have only one sample,
# but measurements corresponding to before and after a treatment. Therfore we 
# will want to be able to group based on the type of treatment and the time of
# the recording. In this case that would mean creating groups for the pilocarpine 
# drug trials for the control recordings (before drug treatment) and the
# experimental recordings (after drug treatment).

# This concept of grouping can be accomplished using the "subset" function. 
# Subsetting allows you to establish your own subsets within the data that you 
# can reference within tests. 
...
# Now that we have our subsets of interest, lets go ahead and visualize them to 
# get an idea of their distributions visually. This can be done using the 
# "simple.eda" function which will construct a few different graphs portraying
...
#Since both of our p-values are greater than .05 we fail to reject the null 
# hypothesis that the data is normally distributed. Therefore we can now proceed 
# with statistical tests designed to investigate normally distributed data sets.

#Had our sample not been normally distributed we would have to select different
#statistical tests, but I will elaborate on that more later. 

#Ultimately we will be testing the null hypothesis that there is no difference
#in the spike rate before and after the application of the drug pilocarpine. 
...
# #A paired t-test will allow us to probe these hypotheses using the "t.test"
# function since the sample was found to be normally distributed. 

# You must specify the pre and post measurements of the paired sample that you will
# be comparing, followed by "paired" so that it knows not to treat the data 
# as two independent samples. 

# Finally you must specify the alternative hypothesis as either "two.sided" -
# to test if the pre and post readings are different without specifying the 
# direction of the difference, "greater" - as we will in this case to test if
# the post readings are greater than the pre readings, or "less" - to test if
# the post readings are less than the pre readings.
...
#***Had the data instead not been found to be normally distributed from our 
# shapiro test we would have to instead perform a wilcox rank sum test, this 
# test is non parametric meaning it is robust to non-normal data sets. 
...
# would be identical to the ones from their respective initial EDA's. This
# is not what we want as these are not two independent samples, but rather
# a paired data set from a single sample. To visualize the DIFFERENCE between
# the paired recordings let's create a single variable representing the differences. 
...
# test, and finally interpretting the results of that test with regards 
# to your initial hypotheses.


#Applications and Questions:

# ****IN this hypothesis testing section we took you through how to test a 
# given hypothesis. Now it's your turn! Develop your own hypothesis and refer
# back to this example to guide your investigation of it. You could potentially 
...
# 3. Based on the output from your shapiro test, what type of statistical test
...
# 5. How will you visualize your findings? Which of the graphs introduced earlier
# will best demonstrate your analysis?
