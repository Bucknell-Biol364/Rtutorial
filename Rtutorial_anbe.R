...
# An R script file (such as this one you are working with) defaults to treat any text typed as code to be run. You can also write things in R that are meant to be comments by putting the pound symbol # before it. Everything in this tutorial that is meant to be read as guide will be commented out with a #.
...
# To create a working directory you should first create a folder in a location on this computer that you can readily access. We recommend creating the folder in Documents. You should already have a zip folder with all the files for this tutorial saved in either Documents or Downloads, so you don't really have to create a new folder. Next select the "Session" tab located above this window near where it says "File" and "Edit". Next click "Set Working Directory" and select "Choose Directory..." from the drop down menu. Within the file explorer that will appear, navigate to where you stored the new folder you just created. Select the folder and click "Ok" to set it as your working directory.

# You can also set the working directory with code using the function below and typing out the full file path.

# Please change the code below to be accurate based on where your folder is and what you named it.
...
# Next we want some data to play with! There are a ton of great sample data sets avalable online and even a few built in ones here in R that you can practice with, but for this tutorial we will be using the data from behavioral observations on the capuchins conducted by Prof. Gazes' lab. This data will be similar to those you will obtain when conducting behavioral observations (obs) this semester using the Animal Observer application on an iPad. The data in the file "CapuchinObs.csv" has been collected for years so although it will be larger than what you will use in class, the output will look similar. It includes information on how long the observation lasted for, which monkey was observed (focal animal), what behaviors they did, how long each behavior lasted, who they were interacting with, and several more pieces of information.
...
# To work with a data set you will want to read it into the R session as a data frame. A data frame is a table of information that can be of various types which is convenient when data are not of uniform classes. The read.csv function automatically takes the file name you provide in quotes and creates a data frame named whatever you provide to the left of the command containing the information from your data file. Below you can see the code in action. The View function allows you to look at data set in a new tab in this R window.
...
# Here is some generic code for exploring a data set in a file that is named DataFile. You can change DataFile to Obs in the code below to explore the data from the csv file.
...
# Before further exploring the data set, one issue you may run into with the output from the iPad is that it may include information you should not analyze. This is because data collected during obs can be for training purposes or real data to be analyzed for research. You can also use the iPad to record reliability data which is when multiple people observe the same behaviors and the data will be compared to see how similar they are. Here, we will mainly be concerned with data for research. The "Use" column in the csv file will stipulate what the data was collected for. Prof Gazes primarily wants you to analyze data meant for Research and to ignore the rest. You can do this by creating a subset of the data and making it into a new data frame.
...
# You can also focus on a specific value of a variable with the format DataFile$Variable=='Value'.
...
# You can check to see if the data is normal using the simple.eda function in the UsingR package. The simple.eda function will show you a histogram, boxplot, and normal Q-Q plot of the data. You will first need to install the package, read the package into the R session using the library function, and then you can use that function.
...
# For practice let's test the hypothesis that a juvenile will have longer play bouts than an adult. For this hypothesis we will choose to focus on one juvenile (Nova) and one adult (Newton).

# To start it would be helpful to group the data based on the variables you are most interested in. The variables of interest would be in the Focal Duration, Behavior, Actor, Subject, and Duration columns.

# Below is some code to first create a dataframe for every obervation where Nova is either the Actor (initiates behavior) or Subject (receives behavior) during Social Play. The same code but for Newton is below.
...
# Now that we have two subsets for how much Nova played and how much Newton played, lets go ahead and visualize them to get an idea of their distributions visually. This can be done using the "simple.eda" function which will construct a few different graphs portraying the data in different ways. 
...
# Newton's play data has a p-value greater than .05 so we fail to reject the null hypothesis that the data is normally distributed. However, Nova's data rejected the null so we can attempt to log transform the data to see if it will make it normal. We will use the natural log and log base 10.

shapiro.test(log(NovaPlay$Duration))$p.value
shapiro.test(log10(NovaPlay$Duration))$p.value

# Neither of these transformations made Nova's data normal, so we will proceed with statistical tests designed to investigate data sets that are not normally distributed. These tests are called non-parametric.

# Had our sample been normally distributed we would have to select different statistical tests, called parametric which assumes the data is normal.

# Ultimately we will be testing the null hypothesis that there is no difference in the play duration of Nova and Newton, a juvenile and an adult. This can be written more concisely as:
...
# The HA utilizes absolute values to represent that the difference could be negative if Newton plays more than Nova, but if the absolute value is larger than 0 then it would still show that there is a difference.

# If both data sets were normal then we would have run a t-test for two independent samples, but because we need to use non-paramtric tests we will analyze the data using a Wilcoxon Mann Whitney test. The url below will direct you to a helpful guide online for picking the correct statistical test and provides the R code to run it.

# https://stats.idre.ucla.edu/other/mult-pkg/whatstat/
  
wilcox.test(NovaPlay$Duration, NewtonPlay$Duration)

# We see from the output of our test that the p-value is greater than .05 (p = 0.6671). This leads us to fail to reject our null hypothesis that there is no difference in the duration of social play for Nova and Newton from our capuchin group. Therfore we can state that the juvenile and adult we tested play for similar lengths of time.

# We can visualize the amount of play by Nova and Newton using a boxplot. This visualization works well when you have one categorical and one numeric variable. In this case, the individual monkey will be categorical and the amount of play is numeric.

# Now we can create a boxplot of these differences with the following code:
  
boxplot(NovaPlay$Duration, at = 1, xlim = c(0.5, 2.5), 
        ylim = range(c(NovaPlay$Duration, NewtonPlay$Duration)), xlab = "Capuchin", ylab = "Social Play Duration",main = "Juvenile and Adult Social Play")
...
# Now try it out with a different hypothesis using a different behavior or subjects from the initial data set!
