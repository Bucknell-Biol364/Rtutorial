...
R is a program for data science. It is an extremely useful tool for manipulating, visualizing, and investigating data. It will allow you to explore data to formulate hypotheses and then go one step further to test them with statistical tests.  

However, before we can do any of these data visualizations or analyses we wil first need data sets and a place to store and access both the data and the code you write to investigate them. 

There are a ton of great sample data sets avalable online and even a few built in ones here in R, but for this course will will be using a data set from a lab similar to those you will be performing. The data set "neuro_253_Cleaned_Data" is a series of extracellular neuronal recordings from crayfish. It includes recordings of "spikes" which refer to firing of the neurons measured both before and after treatment with drugs. 

This file is in the .csv format, an abbreviated for "comma separated values". You can create such a file from an excel document by selecting "save as" and the .csv format. 
...
Since both of our p-values are greater than .05 we fail to reject the null hypothesis that the data is normally distributed. Therefor we can now proceed with statistical tests designed to investigate normally distributed data sets.
...
A paired t-test will allow us to probe these hypotheses using the "t.test"" function since the sample was found to be normally distributed.
...
We see from theoutput of our paired t-test that the p-value is less than .05. This leads us to reject our null hypothesis that the drug had no effect on spike rate. Therfore we can state that the drug pilocarpine significantly increased spike rate. 
